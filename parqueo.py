...
class Parqueo:
    def __init__(self, configuracion, clientes_manager=None, reservas_manager=None):
        ...
        self.clientes = clientes_manager
        self.reservas = reservas_manager
    ...
    def meter_vehiculo(self, placa, tipo):
        ...
        # Verifico si existe reserva activa para esta placa
        if self.reservas:
            reserva = self.reservas.buscar_activa_por_placa(placa)
            if reserva:
                # Marcar reserva usada al momento de ingresar
                ok, res = self.reservas.marcar_usada_por_placa(placa)
                if ok:
                    print("\nSe detectó reserva activa para esta placa: ",placa)
                else:
                    print("\nSe detectó reserva pero no pudo marcarse como usada:", res)
                # seguir ingreso normal (reserva no garantiza espacio automático)
